...
async function garantirAuthAnonima(){ return true; }
...
async function loginFirebaseEmail(email, senha){
  const cred = await signInWithEmailAndPassword(auth, String(email||'').trim().toLowerCase(), senha);
  state.user = cred.user;
  await garantirPerfilUsuarioLogado();
...
async function iniciarSessaoAutenticada(){
...
async function carregarPerfil(){
  if(!state.user?.uid){ await logoutFirebase(); return; }
  const ref=doc(db,colecoes.usuarios,state.user.uid);
  ...
  if(state.profile.ativo===false){ toast('Usuário desativado. Procure o administrador.','error'); await logoutFirebase(); return; }
...
async function criarUsuarioAdmin(ev){
  ...
  if(!nome || !email || !email.includes('@') || senha.length<6){
    toast('Informe nome, e-mail válido e senha padrão com pelo menos 6 caracteres.','error');
    return;
  }
  ...
  try{
    ...
    await setDoc(doc(db,colecoes.usuarios,cred.user.uid),{
      nome, email, role, perfil:role, ativo:true,
    ...
    const msg = err?.code === 'auth/email-already-in-use'
      ? 'Este e-mail já existe no Firebase Authentication.'
      : (err?.message || err);
...
onAuthStateChanged(auth, async user=>{
  ...
  try{
    ...
    toast('Erro ao carregar usuário: '+(err?.message||err),'error');
    await logoutFirebase();
  }
}); 
